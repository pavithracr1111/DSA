#include "stack.h"
//create_stack function definition 
int create_stack(Stack_t *stk, int size)
{
    //allocating the memory based on size 
    stk -> stack = malloc(size * sizeof(int));
    //if stack is not created then return failure
    if(stk -> stack == NULL)
    {
        return FAILURE;
    }
    //store the size in capacity and update the top with -1
    stk -> capacity = size;
    stk -> top = -1;

}

//main function
int main()
{
	int choice, element, peek, size;
	Stack_t stk;

    printf("Enter the size of the stack: ");
    scanf("%d", &size);
    //function call
    create_stack(&stk, size);
	printf("1. Push\n2. Pop\n3. Display Stack\n4. Peek(Element at Top)\n5. Exit\nEnter your choice : ");
    //super while loop
	while (1)
	{
		scanf("%d", &choice);
		switch(choice)
		{
			case 1:
				printf("Enter the element to be pushed in stack : ");
				scanf("%d", &element);
				//function call for push function
				if (Push(&stk, element) == FAILURE)
				{
					printf("INFO : Stack Full\n");
				} 
				break;
			case 2:
			    //function call for pop 
				if (Pop(&stk) == FAILURE)
				{
					printf("INFO : Stack is empty\n");
				}
				else
				{
				    printf("INFO : Pop operation is successfull\n");
				}
				break;
			case 3:
			    //function call for peep function
				Peep(stk);
				break;
			case 4:
			    //function call for peek function
				if ((peek = Peek(&stk)) == FAILURE)
				{
					printf("INFO : Stack is empty\n");
				}
				else
				{
					printf("INFO : Peek element is %d\n", peek);
				}
				break;
			case 5:
				return SUCCESS;
			default:
				printf("Invalid Choice.\n");
				break;
		}
	}
	return 0;
}